...
#         os.system('wget -q -P {} {}'.format('./dataset/raw_zinc_smiles', url))
        urllib.request.urlretrieve(url, './smiles_data/fragment/{}'.format(name))
        return name, True
    return name, False
...
def log_result(output):
    name = output[0]
    flag = output[1]
    cnt = countline('./smiles_data/fragment/{}'.format(name))
...
def save_smi(start_offset=0, filename='./smiles_data/fragment/ZINC-downloader-2D-smi-fragment.uri'):
    ts = time.time()
